...
def schedule_continuous_logging(action, often, repetition, when: datetime.time):
    ...
    # Create action
    TASK_ACTION_EXEC = 0
    tsk_action = task_def.Actions.Create(TASK_ACTION_EXEC)
    tsk_action.ID = 'DO NOTHING'
    tsk_action.Path = path.join(path.abspath(""), 'continuous_logging/Continuous Logging.exe')

    app_path = sys.argv[0]
    console_app_path = path.join(path.abspath(""), "Console Application.exe")

    if action == 'Open Time Logger' and path.exists(app_path):
        tsk_action.Arguments = f'app "{app_path}"'
